...
export const CardComponent = ({ data }) => {
    ...
    const drawStars = () => {
        ...
        for (let i = 0; i < 5; i++) {
            const star =
                (i < Math.ceil(Math.min(data.vote_average, 5))) ?
                    (<IoMdStar key={i} />) : (<MdOutlineStarBorderPurple500 key={i} />)
            stars.push(star)
        }
        return stars
                        ...
                        <div className="content align-self-center position-absolute top-50 start-50 translate-middle h-100 w-100 p-3">
                            <div className="centered d-flex flex-column justify-content-center align-items-center h-100 text-center">
                                ...
                                <div className="d-flex">
                                    <span>Language: </span>
                                    <img id="flag" src={`https://flagsapi.com/${element.original_language?.toUpperCase()}/shiny/64.png`} className=" mx-2 card-text"
                                        onError={(e) => e.target.src = "https://flagsapi.com/BE/flat/64.png"}
                                    />
                                </div>
                                <span className="btn btn-dark">{drawStars()}</span>
                            </div>
                        </div>
                    </div>
                </div>
